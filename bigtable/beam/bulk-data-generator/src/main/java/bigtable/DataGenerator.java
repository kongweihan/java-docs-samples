...
public class DataGenerator {
  ...
  public static void main(String[] args) throws IOException, GeneralSecurityException {
    ...
    generateRate = options.getBigtableRate();
  ...
  static void generateDataWithGroupBy(BigtablePipelineOptions options)
    ...
    System.out.println("Generating " + options.getBigtableRows() + " rows, each "
        ...
        + " bytes per row, at rate of " + options.getBigtableRate() + " rows per second.");

    String generateLabel = String
        .format("Generate %d rows at %d rows per second for table %s", numRows, generateRate,
    ...
    PCollection<Long> numbers = p.apply(generateLabel, GenerateSequence.from(0).to(numRows)
        .withRate(generateRate, Duration.standardSeconds(1)));
    ...
    PCollection<Mutation> mutations = expanded.apply(mutationLabel,
    ...
    mutations.apply(
        ...
        CloudBigtableIO.writeToTable(new CloudBigtableTableConfiguration.Builder()
            ...
            .withConfiguration(BigtableOptionsFactory.BIGTABLE_BULK_MAX_ROW_KEY_COUNT, "15")
  ...
  static class CreateMutationFn extends DoFn<Long, Mutation> {
    ...
    @ProcessElement
    public void processElement(@Element Long number, OutputReceiver<Mutation> out) {
  ...
  public interface BigtablePipelineOptions extends DataflowPipelineOptions {
    ...
    @Description("The generating rate")
    Integer getBigtableRate();

    void setBigtableRate(Integer bigtableRate);
